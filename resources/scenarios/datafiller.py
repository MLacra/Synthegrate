...
# -*- coding: utf-8 -*-
#
# $Id: datafiller.py 853 2017-08-02 19:08:46Z fabien $
#
# TODO:
#
# - set MANDATORY parameter list in class to factor out checking
# - generate csv into separate files?
# - accept UTF8 object names
# - formula and constants: mult=#Medecin*#Duree*200*30
# - generate packed binary data? option to set a file prefix?
# - prefix db specific options with the db name, eg --pg-freeze?
# - add sqlite support based on csv?
...
# - CREATE UNIQUE INDEX would be useful to parse?
...
# - int: add rot(ation)? bit permutation? sbox?
...
# - allow to use "faker" python lib? gen=faker.function
# - allow expressions? python eval?
#
# NOT TO DO:
...
#   alas nope, python threads are a joke.

# see other python2/3 compatibility hacks below...
from __future__ import print_function

VERSION = '2.0.1-dev'
Id='$Id: datafiller.py 853 2017-08-02 19:08:46Z fabien $'
...
=item C<--csv-null=NULL>

Set NULL encoding for CSV output.

Default is C<NULL>.

=item C<--csv-separator=SEP>
...
=item C<--target (postgresql|mysql|csv|tsv)> or C<-t ...>

Target database engine.
PostgreSQL support is well tested.
MySQL support is more experimental.
CSV (comma-separated values) and
TSV (tabulation-separated values) support is partial.
...
=head2 GENERATORS AND DIRECTIVES
...
foreign keys... These informations are used to set up a default generator
for each field. The generator parameters can be modified by providing
additional directives.

The schema can be augmented with B<directives> in comments so as to provide
hints about data generation. A datafiller directive is a special SQL comment
recognized by the script, with a C<df> marker at the beginning.
A directive must appear I<after> the object about which it is applied, either
...
Add C<--freeze> and C<--no-freeze> options to trigger PostgreSQL
C<COPY FREEZE> option.
Add license information in generated output.
Add support for generating CSV and TSV (with inputs from Alex Reece).
Improved MySQL support by Alex Reece and Robert Lee (datetime, bigint...).
...
def run_unit_tests(seed=None):
    ...
    for v, lt in UNITS:
        ...
        if ok != (status == 0):
            ...
            fail += 1
    if fail:
        sys.stderr.write("unit tests failed: %s\n" % fail)
    sys.exit(fail)
...
RE_BLO=r'BYTEA|BLOB'
RE_INT=r'%s|(BIG|TINY|SMALL|MEDIUM)INT|INT[248]|INTEGER|INT\b' % RE_SER
...
RE_INTV=r'INTERVAL'
RE_TIM=r'DATE|%s|%s' % (RE_TSTZ, RE_INTV)
RE_BOO=r'BOOL(EAN)?'
RE_IPN=r'INET|CIDR'
RE_MAC=r'MACADDR'
...
# and arrays thereof
RE_ALLT = r'(%s)%s' % (RE_TYPE, RE_ARRAY)

# SQL syntax
new_object = re.compile(r"^\s*(%s)\s" % RE_CMD, re.I).match
# 1=table name, [2, 3]
create_table = \
    re.compile(r'^\s*CREATE\s+TABLE\s*(%s)\s*\(' % RE_IDENT2, re.I).match
# 1=type name, [2, 3]
create_enum = \
    re.compile(r'^\s*CREATE\s+TYPE\s+(%s)\s+AS\s+ENUM' % RE_IDENT2,
               re.I).match
# 1=type
create_type = \
    re.compile(r'\s*CREATE\s+TYPE\s+(%s)\s+AS' % RE_IDENT2, re.I).match

# 1=?, 2=column, 3=type
COLUMN = r'^\s*,?\s*(ADD\s+COLUMN\s+)?(%s)\s+(%s)' % (RE_IDENT, RE_ALLT)
column = re.compile(COLUMN, re.I).match
is_int = re.compile(r'^(%s)$' % RE_INT, re.I).match
is_serial = re.compile(r'^(%s)$' % RE_SER, re.I).match
# enums are added when definitions are encountered
column_enum = None

REFERENCE = r'.*\sREFERENCES\s+(%s)\s*(\((%s)\))?' % (RE_IDENT2, RE_IDENT)
# 1=table, [2, 3], 4=?, 5=dest columns (?)
reference = re.compile(REFERENCE, re.I).match
primary_key = re.compile('.*\sPRIMARY\s+KEY', re.I).match
unique = re.compile(r'.*\sUNIQUE', re.I).match
not_null = re.compile(r'.*\sNOT\s+NULL', re.I).match
UNICITY = r'.*(UNIQUE|PRIMARY\s+KEY)\s*\(([^\)]+)\)'
# 1=unicity type, 2=columns
unicity = re.compile(UNICITY, re.I).match
# 1=?, 2=table name, [3, 4]
alter_table = \
    re.compile(r'\s*ALTER\s+TABLE\s+(ONLY\s+)?(%s)' % RE_IDENT2,
               re.I).match
ADD_CONSTRAINT = r',?\s*ADD\s+CONSTRAINT\s+(%s)\s+' % RE_IDENT
# 1=constraint name, 2=unicity type, 3=columns
add_unique = re.compile(ADD_CONSTRAINT + UNICITY, re.I).match
# 1=constraint_name, 2=source columns, 3=table, [4, 5], 6=, 7=dest columns
add_fk = \
    re.compile(ADD_CONSTRAINT + r'FOREIGN\s+KEY\s*\(([^\)]+)\)' + REFERENCE,
               re.I).match
# 1=column
alter_column = re.compile(r',?\s*ALTER\s+COLUMN\s+(%s)' % RE_IDENT, re.I).match

# DETECT DATAFILLER DIRECTIVES
# commented-out directives, say '--- df...' or '---- df...' or '-- -- df...'
df_junk = re.compile('.*?--.*-\s*df.*').match
# simple directive: 1=contents
df_dir = re.compile(r'.*--\s*df[^:]*:\s*(.*)').match
# macro definition: 1=name, 2=contents
df_mac = re.compile(r'.*--\s*df\s+([\w\.]+)\s*:\s*(.*)').match
# explicit table: 2=name
df_tab = \
    re.compile(r'.*--\s*df[^:]*\s+(t|table)=({0})(\s|:)'. \
               format(RE_IDENT2), re.I).match
# explicit attribute: 2=name
df_att = \
    re.compile(r'.*--\s*df[^:]*\s+(a|att|attribute)=({0})(\s|:)'. \
               format(RE_IDENT), re.I).match
# string/float/int directives: 1=name 2=value 3=reminder
df_txt = re.compile(r'(\w+)=\'([^\']*)\'\s+(.*)').match
df_flt = re.compile(r'(\w+)=(-?\d+\.\d*)\s+(.*)').match
df_int = re.compile(r'(\w+)=(-?\d+)\s+(.*)').match
df_str = re.compile(r'(\w+)=(\S*)\s+(.*)').match
# simple directive: 1=name 2=reminder
df_bol = re.compile(r'(\w+)\s+(.*)').match

# remove SQL comments & \xxx commands
comments = re.compile(r'(.*?)\s*--.*').match
backslash = re.compile(r'\s*\\').match
...
def unescape(s, regexp=False):
    ...
    while i < len(s):
        ...
        if c == '\\':
            assert i+1 < len(s), \
                "escaped string '%s' must not end with '\\'" % s
...
class Generator(object):
    ...
    def __init__(self, att, params=None):
        ...
        # show generators
        if opts.debug:
            debug(1, "%s: %s %s" % (att, generator_count, self.__class__))
            debug(2, "params: %s" % strDict(self.params))
        self.type = self.params['type'].lower() if 'type' in self.params else \
                    att.type if att else \
                    None
        # null probability
        self.nullp = 0.0
        if self.att is None: # for tests
            self.nullp = \
    ...
    def __str__(self):
        if self.att:
            return "%s %s" % (self.__class__.__name__, self.att)
        else:
...
class NULLGenerator(Generator):
    ...
    def __init__(self, att=None, params=None):
        ...
        if 'null' in p:
            assert p['null'] == 1.0, \
                "%s: 'null' is %s instead of 1.0" % (self, p['null'])
...
class WithLength(Generator):
    ...
    def __init__(self, lenmin=None, lenmax=None):
        ...
        assert not (mm and lv), \
            "%s: not both 'length'/'lenvar' & 'lenmin'/'lenmax'" % self
        if self.type is not None and not mm and not 'length' in self.params:
            ...
            if clen:
                ...
                if re.match('var(char|bit)', self.type):
                    ...
                    assert self.params.get('lenvar', 0) == 0, \
                        "%s: non zero 'lenvar' on CHARS(*)" % self
        ...
        if self.lenmin is not None and self.lenmax is None:
            self.lenmax = int(self.lenmin * 4 / 3)
        elif self.lenmax is not None and self.lenmin is None:
            self.lenmin = int(self.lenmax * 3 / 4)
        elif self.lenmin is None and self.lenmax is None:
            self.lenmin, self.lenmax = lenmin, lenmax
    ...
    def __init__(self, name=None, att=None, p={}, g=None, backup=False):
        if g:
            ...
            assert (p and name in p) or backup, \
                "%s: mandatory '%s' directive" % (self, name)
            if p and name in p:
...
class CharsGenerator(WithSubgen, StringGenerator):
    ...
    def parseCharSequence(obj, s):
        ...
        while d != -1:
            if d == 0:
                ...
                assert d < len(c)-1, \
                    "%s: '%s' cannot end with dash" % (obj, s)
    ...
    def __init__(self, att=None, params=None, chars=None):
        ...
        if att is not None and att.isUnique():
        ...
        assert len(self.chars) > 0, \
            "%s: no characters to draw from" % self
...
class EANGenerator(LuhnGenerator):
    ...
    def __init__(self, att=None, params=None):
        ...
        assert re.match(r'M?\d*', self.prefix), \
            "%s: invalid 'prefix' %s" % (self, self.prefix)
        assert len(self.prefix) < self.length, \
            "%s: 'prefix' \"%s\" length must be smaller than 'length' %s" % \
                (self, self.prefix, self.length)
...
class WordGenerator(IntGenerator):
    """Generate words, from a list or file
    ...
    def __init__(self, att, params=None, words=None):
        ...
        if words:
            ...
            assert len(spec) > 0, "%s: empty word specification" % self
            if spec[0] == ':':
                ...
                f = open(spec)
                assert f, "%s: cannot open '%s'" % (self, spec)
    ...
    def setSize(self, size):
        ...
        if self.offset + self.size > len(self.words):
            sys.stderr.write("{0}: offset={1} shifted to keep in bound (size={2}, len={3})\n".
                             format(self.att, self.offset, self.size, len(self.words)))
...
class FileGenerator(IntGenerator):
    ...
    def __init__(self, att, params=None):
        ...
        import os, glob
        self.files = []
        assert 'file' in self.params, "%s: no directive 'file'" % self
        ...
        assert mode == 'blob' or mode == 'text', \
            "%s: mode must be 'blob' or 'text'" % self
        self.conv = bytes if mode == 'blob' else \
                    lambda s: s.decode(opts.encoding)
        # set size & offset
        assert len(self.files) > 0, "%s: non empty set of files" % self
        if self.size is None:
    ...
    def __init__(self, att, params=None):
        ...
        if self.network.find(':') != -1:
            ...
            try:
                ...
                assert False, "%s: invalid ipv6 %s" % (self, self.network)
            self.net = ip.first
            ...
            assert re.match(r'(\d+\.){3}\d+$', ipv4), \
                "%s: invalid ipv4 %s" % (self, self.network)
            try:
                n, m = 0, int(m)
                assert 0 <= m and m <= 32, \
                    "%s: ipv4 mask %d not in 0..32" % (self, m)
                # extract network number
                for i in map(int, re.split('\.', ipv4)):
                    assert 0 <= i and i <= 255, \
                        "%s: ipv4 byte %s not in 0..255" % (self, i)
                    n = n * 256 + i
            except:
                assert False, "%s: invalid ipv4 %s" % (self, self.network)
            assert 0 <= n and n <= 0xffffffff, \
                "%s: ipv4 address %s not on 4 bytes" % (self, n)
            self.mask = (1 << (32-m)) - 1
            self.net = n & (0xffffffff ^ self.mask)
        # override size & offset if was not set explicitely
        size = self.mask - 1 + int(beg) + int(end)
        if self.size is None:
            assert size > 0, "%s: empty address range %s" % (self, self.network)
            self.setSize(size)
        else:
...
class AltGenerator(RandomGenerator):
    ...
    def __init__(self, att=None, params=None, gens=None):
        ...
        if gens is not None:
            ...
            assert 'alt' in self.params, "%s: mandatory 'alt' directive" % self
            self.alts = []
            self.weight = []
            self.total_weight = 0
            ...
            for mw in self.params['alt'].split(','):
                if ':' in mw:
                    ...
                    m, w = mw, 1
                assert w > 0, "%s: weight %s must be > 0" % (self, w)
    ...
    def __init__(self, att, params=None, gen=None):
        ...
        if 'extent' in p:
            assert \
                ...
                "%s: both 'extent' and 'len*' directives" % self
            extent = p['extent']
            assert re.match(r'\d+(-\d+)?$', extent), \
                "%s: bad 'extent' %s" % (self, extent)
            if extent.find('-') != -1:
            ...
            assert 0 <= min and  min <= max, \
                "%s: extent must be 0 <= %s <= %s" % (self, min, max)
        else:
            min, max = 1, 1
...
class Pattern:
    ...
    def skipRepeat(pattern, i=0):
        ...
        if i < len(pattern):
            if pattern[i] == '{':
                ...
                assert i != 0, "%s: closing '}' not found" % pattern
                # assert re.match('\{\d+(,\d+)?}', ...)
            elif pattern[i] in '?+*':
                i += 1
        return i
    ...
    def catSplit(pattern):
        ...
        if pattern[0] == '(':
            ...
            opened, i = 1, 1
            while opened > 0:
                assert i < len(pattern), "%s: no matching ')'" % pattern
                if pattern[i] == ')':
                    opened -= 1
                elif pattern[i] == '(':
                    opened += 1
            ...
            while i < len(pattern):
                if pattern[i] == '\\' and pattern[i+1] in RECHARS:
                    ...
                    assert i < len(pattern), \
                        "%s: cannot end on escape" % pattern
                elif pattern[i] in '{?+*':
                    assert i != 0, \
                        "%s: cannot start with repeat" % pattern
                    if i == 1:
                        # F{3}
                        end = Pattern.skipRepeat(pattern, i)
                        return [ [ pattern[0:i], pattern[i:end] ] ] + \
    ...
    def altSplit(pattern):
        ...
        assert pattern[0] == '(' and pattern[-1] == ')', \
            "%s is not an alternation" % pattern
    ...
    def genCat(att, pattern, extent=''):
        ...
        if len(pats) == 0:
            assert extent == '', \
                "%s: extent %s on nothing" % (pattern, extent)
            return ConstGenerator(att)
        elif len(pats) == 1:
...
class PatternGenerator(RandomGenerator):
    ...
    def __init__(self, att, params=None):
        ...
        assert 'pattern' in self.params, \
            "%s: mandatory 'pattern' directive" % self
...
class ValueGenerator(Generator):
    ...
    def __init__(self, att, params):
        Generator.__init__(self, att, params)
        assert 'value' in self.params, "%s: mandatory 'value' directive" % self
    ...
    def getGenerator(obj, name):
        if not name in SharedGenerator._generators:
            assert name in df_macro, \
                "%s: 'share' %s directive must be a macro" % (obj, name)
            SharedGenerator._generators[name] = \
                SharedGenerator(name, df_macro[name])
        return SharedGenerator._generators[name]
    def __init__(self, name, params):
        assert name is not None and params is not None, "mandatory parameters"
        ...
        assert self.size is not None, \
            "%s: size not set in macro %s" % (self, name)
        # persistent
        WithPersistent.__init__(self)
...
def createGenerator(a, t, params=None, msg=None):
    ...
    """
    if params is None: params = a.params
    if not t:
        t = findGenerator(msg, params)
    if not t and 'type' in params:
        t = findGeneratorType(params['type'])
    assert t, "no generator in %s" % (msg if msg else a)
    assert t in GENERATORS, \
        "unexpected generator '%s' in %s" % (t, msg if msg else a)
    ...
    assert not gen.params, \
        "unexpected '%s' directives: %s" % (t, strDict(gen.params))
    return gen

def macroGenerator(name, att=None):
    assert name in df_macro, \
        "%s: '%s' must be a macro" % (att if att else 'Macro', name)
...
def getParams(dfline):
    ...
    while len(dfline)>0:
        ...
        if d:
            ...
            if d.group(1) == 'use':
                assert d.group(2) in df_macro, "defined macro %s" % d.group(2)
        ...
        if d:
            ...
            dfline = d.group(2)
            continue
        raise Exception("cannot parse: '%s'" % dfline)
    return params
...
class Model(object):
    ...
    def checkPARAMS(obj, params, PARAMS):
        ...
        for k, v in params.items():
            assert k in PARAMS, "unexpected parameter '%s'" % k
            if PARAMS[k] == float:
                assert type(v) is float or type(v) is int, \
                    "type %s instead of float parameter '%s'" % \
                        (v.__class__.__name__, k)
            elif PARAMS[k] == int:
                assert type(v) is int, \
                    "type %s instead of int for parameter '%s'" % \
                        (v.__class__.__name__, k)
            elif PARAMS[k] == bool:
                assert type(v) is bool, \
                    "type %s instead bool for parameter '%s'" % \
                        (v.__class__.__name__, k)
        ...
        assert not ('mult' in params and 'size' in params), \
            "%s: must not have both 'mult' and 'size'" % obj
...
def findGenerator(obj, params):
    ...
    for k in sorted(params):
        if k in GENERATORS:
            assert not found, \
                "%s: multiple generators '%s' and '%s'" % (obj, found, k)
            found = k
    return found
...
class Table(Model):
    ...
    def addUnique(self, atts, type=None):
        ...
        if len(atts) == 1:
            ...
            if type is not None and type.lower() == 'unique':
                att.unique = True
            else:
                att.isPK = True
                att.not_null = True
    ...
    def getAttribute(self, name):
        assert name.lower() in self.atts, "%s Attribute %s" % (self, name)
        return self.atts[name.lower()]
    def getPK(self):
        for a in self.att_list:
            if a.isPK:
                return a
        assert False, "%s: no PK found" % self
    def mapGen(self, func):
    ...
    def getData(self):
        ...
        while tries:
            ...
            if collision:
                ...
                return l
        assert False, \
            "%s: cannot build tuple after %d tries" % (self, opts.tries)
...
class Database(object):
    ...
    def dropTable(self, table):
        return "DROP TABLE %s;" % self.quote_ident(table.name)
    def truncateTable(self, table):
        return "DELETE FROM %s;" % self.quote_ident(table.name)
    # types
    ...
    def quoteIdent(self, ident):
        raise NotImplementedError()
    def quoteString(self, string, cmap):
        return ''.join(cmap[c] if c in cmap else c for c in string)
    def quoteLiteral(self, literal):
        return '\'' + self.quoteString(literal, {'\'':r'\'', '\\':r'\\'}) + '\''
    # values
    def null(self):
        raise NotImplementedError()
...
class CSV(Database):
    ...
    def insertBegin(self, table):
        return self._comment("%s: %s" % \
            (table.getName(), opts.csv_separator.join(a.getName() \
                for a in filter(lambda x: x.gen, table.att_list))))
    def insertValue(self, table, value, isLast):
        return opts.csv_separator.join(db.null() if i is None else
               db.quoteLiteral(i) for i in value)
    def insertEnd(self):
        return ''
    def setSequence(self, tab, att, number):
        return ''
...
class PostgreSQL(Database):
    ...
    def insertEnd(self):
        return '\\.'
    def setSequence(self, tab, att, number):
        name = "%s_%s_seq" % (tab.name, att.name)
        if tab.quoted or att.quoted:
            name = db.quoteIdent(name)
        return "ALTER SEQUENCE %s RESTART WITH %d;" % (name, number)
    def dropTable(self, table):
        return "DROP TABLE IF EXISTS %s CASCADE;" % table.getName()
    def truncateTable(self, table):
        return "TRUNCATE TABLE %s CASCADE;" % table.getName()
    def quoteIdent(self, ident):
...
column_type = None
def addType(t):
    global TYPES, column_type
    if TYPES is None:
        TYPES = t
    else:
        TYPES += '|' + t
    column_type = re.compile(r'^\s*,?\s*(ADD\s+COLUMN\s+)?(%s)\s+(%s%s)' \
                             % (RE_IDENT, TYPES, RE_ARRAY), re.I).match
...
if opts.validate:
    if opts.validate == 'unit':
        ...
        raise Exception("unexpected validation %s" % opts.validate)
...
for line in lines:
    ...
    if d:
        ...
        assert mname not in GENERATORS, \
            "do not use generator name '%s' as a macro name!" % mname
    ...
    if new_object(line) and not alter_column(line):
        current_table = None
        current_attribute = None
        current_enum = None
        att_number = 0
...
if not opts.self_test_hack and opts.target != 'csv':
    ...
    print('')
    print("-- Data generated by: %s" % sys.argv[0])
    print("-- Version %s" % version)
    now = datetime.utcnow().isoformat()
    print("-- For %s on %s (UTC)" % (opts.target, now))
        ...
        if a.FK is not None:
            ...
            if db.textType(a.type):
                assert not 'prefix' in a.params, \
                    "no prefix on FK %s.%s" % (a.table.name, a.name)
...
for t in filter(lambda t: not 'nogen' in t.params, tables):
    for a in t.att_list:
        ...
        if 'nogen' in a.params:
            ...
            a.gen = None
            assert not a.params, \
                "unused '%s' parameters: %s" % (a.name, a.params)
            continue
...
for t in tables:
    ...
    if 'nogen' in t.params or t.size == 0:
        ...
        assert not t.params, \
            "unused %s parameters: %s" % (t.name, t.params)
        db.comment("skip table %s" % t.name)
    else:
        size = "%d*%g" % (t.size, 1.0-t.skip) if t.skip else str(t.size)
        db.comment("fill table %s (%s)" % (t.name, size))
        if not opts.quiet:
            print(db.echo("# filling table %s (%s)" % (t.name, size)))
